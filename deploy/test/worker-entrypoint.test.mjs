...
async function fakeRuntime({ curlScript, fastSleep = false }) {
  ...
  if (fastSleep) await executable(join(bin, "sleep"), "#!/usr/bin/env bash\n/bin/sleep 0.01\n");
  return { root, bin };
}
...
test("worker entrypoint hides transient readiness noise and reports worker exit status", async () => {
  ...
  const result = spawnSync("bash", [entrypoint, "bash", "-c", "exit 17"], {
    cwd: root,
...
test("worker entrypoint emits only the terminal readiness failure", async () => {
  ...
  const result = spawnSync("bash", [entrypoint, "bash", "-c", "exit 0"], {
    cwd: root,
    env: { ...process.env, PATH: `${bin}:/usr/bin:/bin` },
    encoding: "utf8",
    timeout: 10_000,
