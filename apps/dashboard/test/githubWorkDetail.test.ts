...
test("builds GitHub workflow detail from durable transitions and evidence", async () => {
  const detail = await buildGithubWorkDetail({
    ...
    deliveryWorkflows: {
      ...
      listTransitions: async () => [
        { id: "transition-1", workflowId: "workflow-1", stage: "reviewing", attempt: 1, reason: "Reviewing the change.", idempotencyKey: "reviewing-1", details: null, occurredAt: now },
        { id: "transition-2", workflowId: "workflow-1", stage: "reviewing", attempt: 1, reason: "ADE deterministic validation and profile reviews are running.", idempotencyKey: "activity:review-gates", details: { activity: "review-gates" }, occurredAt: "2026-09-03T10:01:00.000Z" },
      ],
    },
  ...
  } as never, project.id, 42, "2026-09-03T10:02:00.000Z");
  ...
  assert.equal(detail.currentProgress?.activity, "review-gates");
  assert.equal(detail.recentProgress[0]?.label, "Running deterministic validation and profile reviews");
  assert.equal(detail.stageStartedAt, now);
});
